...
def place_limit(order_id: int, side: str, price_cents: int, qty: int) -> dict:
...
def place_market(order_id: int, side: str, qty: int) -> dict:
...
def main():
    ...
    for ts, row in hist.iterrows():
        ...
        res = place_limit(order_id, "SELL", ask, qty_base)
        ...
        res = place_limit(order_id, "BUY", bid, qty_base)
        trades = res.get("trades", [])
        if not trades:
            resting.append(order_id)
        order_id += 1
        ...
        if vol > avg_vol * 1.5:
            ...
            res = place_market(order_id, side, mkt_qty)
            trades = res.get("trades", [])
